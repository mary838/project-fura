      ...
      <div className="mx-auto flex w-full max-w-[1200px] flex-col items-center gap-6 lg:gap-[52px]">
        ...
        <RevealGroup className="grid w-full grid-cols-1 gap-3 lg:grid-cols-3 lg:gap-6">
          {PROPERTY_ECOSYSTEM_ITEMS.map((item) => (
            <article
              key={item.title}
              className="group flex h-56 w-full flex-col items-start gap-5 rounded-xl border border-border-primary bg-surface-muted p-8 transition-transform duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] hover:-translate-y-1 hover:shadow-[0_12px_24px_-8px_rgba(9,12,20,0.15)]"
            >
              <div className="flex size-10 shrink-0 items-center justify-center rounded-[20px] bg-surface transition-colors duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:bg-brand-secondary">
